...
impl TypeBase for BoolType {
        ...
        Self: Sized,
    {
        ...
        let bool_value = context.codegen.const_int(int1_type(), value_as_bool.into(), 0);
        let alloca = context.codegen.build_alloca_store(bool_value, int1_type(), &_name);
        Box::new(BoolType {
            name: _name,
    ...
    fn print(&self, astcontext: &mut ASTContext) -> Result<()> {
        ...
        let bool_to_string_func = astcontext.codegen.llvm_func_cache.get("bool_to_str").ok_or(anyhow!("unable to find bool_to_str function"))?;
        let str_value = astcontext.codegen.build_call(bool_to_string_func, bool_func_args, 1, "");
        let print_args: Vec<LLVMValueRef> = vec![str_value];
        let print_func = astcontext.codegen.llvm_func_cache.get("printf").ok_or(anyhow!("unable to find printf function"))?;
