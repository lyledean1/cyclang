...
impl<'a> CodeGenerator<'a> {
    ...
    pub fn generate_expression(&mut self, typed_expr: &TypedExpression) -> Result<GeneratedValue> {
        match typed_expr {
            ...
            TypedExpression::ForStmt {
                ...
                step,
                body,
            } => self.generate_for(var_name, *start, *end, *step, body),
    ...
    fn generate_for(
        ...
        let function = self.builder.current_function.function;
            ...
            let init_value = self.generate_i32(start)?;
            self.set_variable(var_name, init_value);

            // 2. Create basic blocks for the loop
            let loop_cond_block = self.builder.append_basic_block(function, "for_cond");
            let loop_body_block = self.builder.append_basic_block(function, "for_body");
            let loop_incr_block = self.builder.append_basic_block(function, "for_incr");
            let loop_exit_block = self.builder.append_basic_block(function, "for_exit");

            // Jump to condition block
            self.builder.build_br(loop_cond_block);

            // 3. Generate loop condition
            self.builder.set_current_block(loop_cond_block);

            // Load current value of loop variable
            let var = self
                .get_variable(var_name)
                .ok_or_else(|| anyhow!("Loop variable {} not found", var_name))?;
            let var_ptr = var
                .pointer
                .ok_or_else(|| anyhow!("Loop variable has no pointer"))?;
            let current_val = self
                .builder
                .build_load(var_ptr, int32_type(), "for_var_load");

            // Create condition: i < end (or i > end for negative step)
            let end_val = self.builder.const_int(int32_type(), end as c_ulonglong, 0);
            let end_ptr = self
                .builder
                .build_alloca_store(end_val, int32_type(), "for_end");
            let cmp_op = if step > 0 { "<" } else { ">" };

            // Build comparison
            let lhs_num = crate::types::num::NumberType {
                name: var_name.clone(),
                llvm_value: current_val,
                llvm_value_pointer: Some(var_ptr),
            };
            let rhs_num = crate::types::num::NumberType {
                name: "end".to_string(),
                llvm_value: end_val,
                llvm_value_pointer: Some(end_ptr),
            };
            let cond_result =
                self.builder
                    .cmp(Box::new(lhs_num), Box::new(rhs_num), cmp_op.to_string())?;
            let cond_val = cond_result.get_value();
            ...
            self.builder.set_current_block(loop_body_block);
            self.generate_expression(body)?;
            self.builder.build_br(loop_incr_block); // Jump to increment

            // 5. Generate increment
            self.builder.set_current_block(loop_incr_block);
            ...
            let current_val = self
                .builder
                .build_load(var_ptr, int32_type(), "for_var_load_incr");

            // Add step to current value
            let step_val = self
                .builder
                .const_int(int32_type(), step.unsigned_abs() as c_ulonglong, 0);
            let lhs_gen = GeneratedValue {
                value: current_val,
                pointer: Some(var_ptr),
                ty: ResolvedType::I32,
            };
            let rhs_gen = GeneratedValue {
                value: step_val,
                pointer: None,
                ty: ResolvedType::I32,
            };
            let op = if step > 0 { "+" } else { "-" };
            let new_val = self
                .builder
                .arithmetic_v2(&lhs_gen, &rhs_gen, &op.to_string())?;

            // Store the new value
            self.builder.build_store(new_val.value, var_ptr);
            ...
            // 6. Position at exit block
            self.builder.set_current_block(loop_exit_block);

            // For loops return void
        Ok(GeneratedValue {
            value: current_val, // Dummy value
            pointer: None,
            ty: ResolvedType::Void,
        })
    }
